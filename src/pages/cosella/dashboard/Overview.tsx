...
export default function Overview() {
  ...
  function copyLink() {
    const url = `${window.location.origin}/cosella`
  ...
  return (
    <div className="mx-auto max-w-4xl px-10 py-12">
      ...
      <div className="mt-6 flex items-center justify-between rounded-2xl bg-gradient-to-r from-[#3a0f1a] to-[#7a1f3d] px-6 py-5">
        <div>
          <p className="text-sm font-semibold text-rose-300 uppercase tracking-wide">Cosella App</p>
          <p className="mt-0.5 text-base font-bold text-white">Download &amp; share Cosella</p>
          <p className="mt-1 text-sm text-white/60">Share the link with your closers so they can download and activate Cosella.</p>
        </div>
        <div className="ml-6 flex flex-shrink-0 gap-3">
          ...
          <button
            onClick={() => navigate(`/cosella/app?email=${encodeURIComponent(adminEmail)}`)}
            className="flex items-center gap-2 rounded-lg bg-rose-400 px-4 py-2 text-sm font-semibold text-[#3a0f1a] transition-colors hover:bg-rose-300"
          >
      ...
      <div className="mt-8 grid gap-5 sm:grid-cols-3">
        <div className="lf-panel p-6">
          <div className="flex items-center gap-2 text-sm font-semibold text-slate-500"><DollarSign className="h-4 w-4 text-rose-500" /> Cash collected</div>
          <p className="mt-3 text-2xl font-black text-slate-900">${cashCollected.toLocaleString()}</p>
        </div>
        <div className="lf-panel p-6">
          <div className="flex items-center gap-2 text-sm font-semibold text-slate-500"><ShieldCheck className="h-4 w-4 text-rose-500" /> Deals saved</div>
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-3">
        {hottestProspects.map(p => (
          <div key={p.id} className="lf-panel p-4">
            ...
            <p className="mt-1 text-xs font-semibold text-rose-600">{p.heatSignal}</p>
          </div>
        ))}
      </div>
      ...
      <div className="mt-4 lf-panel p-6">
        ...
        <div className="mt-2 h-2.5 w-full overflow-hidden rounded-full bg-slate-100">
          <div className="h-full rounded-full bg-rose-500" style={{ width: `${guaranteePct}%` }} />
        </div>
      </div>
    </div>
  )
