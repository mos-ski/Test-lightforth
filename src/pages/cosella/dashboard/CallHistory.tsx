  ...
  if (selected) {
    return (
      <div className="mx-auto max-w-4xl px-10 py-12">
        <button onClick={() => setSelected(null)} className="flex items-center gap-1.5 text-sm font-semibold text-rose-600 hover:underline">
        ...
        <div className="mt-4 flex items-start justify-between gap-4">
          ...
          {selected.outcome !== 'won' && (
            <button
              ...
              className="flex h-9 flex-shrink-0 items-center gap-1.5 rounded-lg bg-rose-600 px-3 text-xs font-semibold text-white hover:bg-rose-700"
            >
            ...
            <tbody>
              {org.calls.map(call => (
                <tr key={call.id} className="lf-table-row cursor-pointer" onClick={() => setSelected(call)}>
                  ...
                  <td className="lf-table-cell text-right text-sm font-semibold text-rose-600">View →</td>
                </tr>
              ))}
            </tbody>
          </table>
