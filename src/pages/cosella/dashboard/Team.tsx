        ...
        <div className="mt-6 lf-panel p-5">
          ...
          <div className="mt-4 flex gap-3">
            <button
              ...
              className="h-9 rounded-lg bg-rose-600 px-4 text-sm font-semibold text-white disabled:opacity-40"
            >
              Add to team
            </button>
                ...
                <td className="lf-table-cell">
                  {member.seatPaid ? (
                    <button
                      ...
                      className="flex items-center gap-1.5 font-mono text-xs font-semibold text-rose-600 hover:underline"
                    >
                      {member.inviteCode}
                      {copiedCode === member.id ? <Check className="h-3 w-3 text-rose-500" /> : <Copy className="h-3 w-3" />}
                    </button>
                  ) : <span className="text-xs text-slate-400">Available once seat is active</span>}
                </td>
                <td className="lf-table-cell text-right">
                  {member.seatPaid ? (
                    <span className="inline-flex items-center gap-1 rounded-full bg-rose-50 px-2.5 py-1 text-xs font-semibold text-rose-600"><Check className="h-3 w-3" /> Active</span>
                  ) : (
                    <button onClick={() => { markMemberSeatPaid(adminEmail, member.id); refresh() }} className="rounded-full bg-rose-600 px-3 py-1 text-xs font-semibold text-white hover:bg-rose-700">
