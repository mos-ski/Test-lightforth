...
import { BG, CARD as CARD_BG, BORDER, ACCENT } from './shared'
...
export default function ProspectCardScreen({ card, onContinue }: { card: ProspectCardData; onContinue: () => void }) {
  return (
    <div className="flex min-h-[580px] flex-col items-center px-10 py-10" style={{ background: BG }}>
      <p className="text-xs font-semibold uppercase tracking-widest text-rose-400">Prospect Card</p>
      ...
      <div className="mt-8 w-full max-w-lg space-y-4">
        <div className="rounded-xl p-4" style={{ background: CARD_BG, border: `1px solid ${BORDER}` }}>
          ...
          <p className="mt-2 text-sm text-white">Watched <span className="font-bold text-rose-400">{card.vslWatchPct}%</span> of the VSL</p>
        ...
        <div className="rounded-xl p-4" style={{ background: CARD_BG, border: `1px solid ${BORDER}` }}>
          ...
          <div className="mt-2 space-y-2">
            {card.openingLines.map((line, i) => (
              <p key={i} className="rounded-lg px-3 py-2 text-sm text-white" style={{ background: 'rgba(244,63,94,0.12)' }}>{line}</p>
            ))}
          </div>
        </div>
      </div>
