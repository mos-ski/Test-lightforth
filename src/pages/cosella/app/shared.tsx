...
export const BG     = '#1a0a10'
export const CARD   = 'rgba(244,63,94,0.07)'
export const BORDER = 'rgba(244,63,94,0.18)'
...
export function CosellaLogo({ size = 28 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 32 32" fill="none">
      ...
      <path d="M16 8v16M12 12.5c0-1.5 1.8-2.5 4-2.5s4 1 4 2.5-1.8 2.5-4 2.5-4 1-4 2.5 1.8 2.5 4 2.5 4-1 4-2.5" stroke="#fb7185" strokeWidth="1.8" strokeLinecap="round" />
    </svg>
  )
}
...
export function CosellaMacWindow({ children, transparency = 0 }: { children: React.ReactNode; transparency?: number }) {
  ...
  const windowBg = `rgba(26, 10, 16, ${bgAlpha})`

  return (
    <div
      className="flex min-h-screen items-center justify-center p-6"
      style={{ background: 'linear-gradient(145deg, #0d0507 0%, #170a0d 50%, #0a0407 100%)' }}
    >
      <div
...
export function Toggle({ on, onToggle }: { on: boolean; onToggle: () => void }) {
  return (
    <button onClick={onToggle} className={cn('relative flex h-6 w-10 flex-shrink-0 items-center rounded-full px-0.5 transition-colors duration-200', on ? 'bg-rose-500' : 'bg-white/20')}>
