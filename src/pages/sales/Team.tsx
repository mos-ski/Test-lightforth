    ...
    <div className="mx-auto max-w-4xl px-10 py-12">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">Add reps by name and email, then activate their seat to send their invite code.</p>
        </div>
        <button
          onClick={() => setShowAddForm(v => !v)}
          className="flex h-10 flex-shrink-0 items-center gap-2 rounded-lg bg-primary px-4 text-sm font-semibold text-white hover:bg-primary/90"
        >
        ...
        <div className="mt-6 lf-panel p-5">
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <input value={email} onChange={e => setEmail(e.target.value)} placeholder={`jordan@${orgDomain}`} className="lf-input" />
              {domainMismatch && (
                <p className="mt-1 text-xs text-destructive">Must be an organizational email — a @{orgDomain} address.</p>
              )}
            </div>
          </div>
          <div className="mt-4 flex gap-3">
            <button
              disabled={!canAdd}
              onClick={() => {
                addMember(adminEmail, { name, email })
                refresh()
                setName('')
                setEmail('')
            ...
            <button onClick={() => setShowAddForm(false)} className="h-9 rounded-lg px-4 text-sm font-semibold text-slate-500 hover:bg-muted">
              Cancel
            </button>
          </div>
        </div>
        ...
        <table className="lf-table">
          <thead className="lf-table-head">
            <tr>
              ...
              <th className="lf-table-th">Role</th>
              <th className="lf-table-th">Invite code</th>
              <th className="lf-table-th text-right">Seat</th>
            </tr>
          </thead>
          <tbody>
            {org.members.map(member => (
              <tr key={member.id} className="lf-table-row">
                ...
                <td className="lf-table-cell capitalize text-slate-600">{member.role}</td>
                ...
                <td className="lf-table-cell text-right">
                  {member.seatPaid ? (
                    ...
                    <button
                      onClick={() => {
                        markMemberSeatPaid(adminEmail, member.id)
                        refresh()
                      }}
