...
import type { CallRecord } from './mockOrg'
...
export default function CallHistory() {
  ...
  if (selected) {
    return (
      <div className="mx-auto max-w-4xl px-10 py-12">
        ...
        <h1 className="mt-4 text-2xl font-bold text-slate-900">Call with {selected.repName}</h1>
        <p className="mt-1 text-sm text-slate-500">{formatDate(selected.date)} · {formatDuration(selected.durationSeconds)}</p>
  ...
  return (
    <div className="mx-auto max-w-4xl px-10 py-12">
      ...
      <p className="mt-1 text-sm text-slate-500">Every Sales Call your team has completed in the desktop app, with the full transcript.</p>

      <div className="mt-8 lf-table-wrap">
        {calls.length === 0 ? (
          <div className="p-10 text-center">
            <Phone className="mx-auto h-8 w-8 text-slate-300" />
            <p className="mt-3 text-sm font-medium text-slate-500">No calls yet — they'll show up here as your team uses Sales Call Copilot.</p>
          </div>
        ) : (
          <table className="lf-table">
            <thead className="lf-table-head">
              <tr>
                <th className="lf-table-th">Rep</th>
                <th className="lf-table-th">Date</th>
                <th className="lf-table-th">Duration</th>
                <th className="lf-table-th text-right">Transcript</th>
              </tr>
            </thead>
            <tbody>
              {calls.map(call => (
                <tr key={call.id} className="lf-table-row cursor-pointer" onClick={() => setSelected(call)}>
                  <td className="lf-table-cell font-medium text-slate-900">{call.repName}</td>
                  <td className="lf-table-cell text-slate-600">{formatDate(call.date)}</td>
                  <td className="lf-table-cell text-slate-600">{formatDuration(call.durationSeconds)}</td>
